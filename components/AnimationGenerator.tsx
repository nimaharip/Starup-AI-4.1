...
import { fileToBase64, delay, getGeminiApiKey, hasGeminiApiKey } from '../utils';
import ApiKeyAlert from './ApiKeyAlert';

// --- CONSTANTS ---

const ANIMATION_STYLES = [
  { id: 'pixar', name: '3D Disney Pixar Style', description: 'Cute, vibrant, expressive, high detail, 3D render' },
  { id: 'anime', name: 'Japanese Anime (Ghibli Style)', description: '2D, hand-drawn, lush backgrounds, soft colors, cel shaded' },
  { id: 'realistic', name: '3D Realistic Cinematic', description: 'Unreal Engine 5 style, hyper-realistic, dramatic lighting, 8k' },
  { id: 'clay', name: 'Claymation (Stop Motion)', description: 'Plasticine texture, Aardman style, tactile feel, handmade' },
  { id: 'lowpoly', name: 'Low Poly 3D', description: 'Minimalist, angular, retro video game aesthetic, geometric' },
  { id: 'watercolor', name: 'Watercolor Animation', description: 'Soft edges, painterly, artistic, dreamy, wet brush texture, paper texture' },
  { id: 'cyberpunk', name: 'Cyberpunk 3D', description: 'Neon lights, futuristic, dark, tech-heavy, volumetric fog' },
  { id: 'paper', name: 'Paper Cutout Style', description: '2D layered craft, textured paper, shadow depth, collage' },
];

const ANIMAJINASI_SCHEMA = `
{
  "system": "Animasi v8.0 - Absolute Consistency",
  ...
  "scene_blueprint_template": {
    "scene_id": "S#",
    "duration_sec": 6,
    "visual_style": "STYLE_PLACEHOLDER. Dialogue must be Indonesian inside [ ... ].",
    "character_lock": {
      "CHAR_A": {
        "name": "Exact name",
...
const analyzeCharacterImage = async (file: File) => {
  if (!hasGeminiApiKey()) {
    throw new Error("Silakan masukkan API Key Gemini Anda di menu Setelan API untuk mulai menggunakan fitur secara gratis.");
  }
  ...
  try {
    ...
    throw new Error("Gagal menganalisa gambar");
  } catch (error) {
...
const generateStoryWithGemini = async (character: any, title: string, model: any, mode = 'initial', existingScenes: any[] = []) => {
  ...
  const schemaWithStyle = ANIMAJINASI_SCHEMA.replace('STYLE_PLACEHOLDER', model.name);
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      const enforcedScenes = parsedData.map((scene: any) => {
        ...
        if (!s.character_lock.CHAR_A.action_flow) {
            s.character_lock.CHAR_A.action_flow = {
                main_action: "Standing still",
                pre_action: "",
                post_action: ""
            };
        }
...
const AnimationGenerator: React.FC = () => {
  ...
  const [selectedModel, setSelectedModel] = useState(ANIMATION_STYLES[0]);
  ...
  const handleGeneratePreview = async (sceneId: number, scene: any) => {
    ...
    const action = char.action_flow?.main_action || "Standing still";
    
    const rawPrompt = `
      Style: ${selectedModel.name}. ${selectedModel.description}.
      Character: ${char.hair || ''}, wearing ${char.outfit_top || ''} and ${char.outfit_bottom || ''}.
      Action: ${action}.
      Setting: ${bg.setting || ''}, ${bg.scenery || ''}, ${bg.lighting || ''}.
      Cinematic framing, high quality, masterpiece.
    `;

    try {
      const imageUrl = await generateImageWithImagen(rawPrompt, aspectRatio);
      setScenes(prev => prev.map(s => s.id === sceneId ? { ...s, generated_image: imageUrl, video_url: null } : s));
    } catch (error: any) {
      console.error(error);
  ...
  const handleGenerateVideo = async (sceneId: number, scene: any) => {
    ...
    try {
      ...
      let prompt = `
        Animation Style: ${selectedModel.name}.
        Character: ${char.hair}, wearing ${char.outfit_top} and ${char.outfit_bottom}.
        Action: ${action}.
        Setting: ${bg.setting}, ${bg.scenery}, ${bg.lighting}.
      ...
      if (!uri) throw new Error("Video URI not returned");
      ...
      if (JSON.stringify(err).includes("RESOURCE_EXHAUSTED") || msg.includes("429")) {
         msg = "Kuota API Habis (429 Resource Exhausted).";
      }
      alert(msg);
    } finally {
  ...
  const copyToClipboardText = (text: string, index: string) => {
  ...
  const getCleanJson = (data: any) => {
    const clean = (item: any) => {
      const { generated_image, video_url, ...rest } = item;
      return rest;
    };
          ...
          <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="bg-[#161922] border border-gray-800 p-8 rounded-3xl space-y-8 shadow-xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                {/* ANIMATION STYLE SELECTOR (NEW) */}
                <div className="col-span-1 md:col-span-2">
                    <label className="block text-sm font-semibold text-gray-400 mb-3 flex items-center gap-2">
                        <Palette className="w-4 h-4 text-purple-400"/> Gaya Animasi (Art Style)
                    </label>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[200px] overflow-y-auto custom-scrollbar pr-2">
                        {ANIMATION_STYLES.map((style) => (
                            <button
                                key={style.id}
                                onClick={() => setSelectedModel(style)}
                                className={`text-left p-3 rounded-xl border transition-all ${
                                    selectedModel.id === style.id
                                    ? 'bg-purple-600/20 border-purple-500 text-white shadow-sm shadow-purple-500/20'
                                    : 'bg-black/30 border-gray-800 text-gray-400 hover:bg-gray-800 hover:border-gray-600'
                                }`}
                            >
                                <div className="font-bold text-sm mb-0.5">{style.name}</div>
                                <div className="text-[10px] opacity-70 line-clamp-1">{style.description}</div>
                            </button>
                        ))}
                    </div>
                </div>
          ...
          <div className="space-y-8 animate-in fade-in pb-20 max-w-4xl mx-auto">
              ...
              <div className="bg-[#161922] border border-gray-800 rounded-2xl p-6 shadow-lg">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <Button 
                    variant="secondary" 
                    onClick={() => copyToClipboardText(JSON.stringify(getCleanJson(scenes), null, 2), 'all')}
                    className="text-sm"
                  >
                 ...
                 {scenes.map((scene, idx) => (
                   <div key={scene.id} className="bg-gray-900/50 border border-gray-800 rounded-xl overflow-hidden transition-all hover:border-gray-700">
                     <div className="flex justify-between items-center bg-gray-800/50 px-5 py-3 border-b border-gray-800">
                       ...
                       <Button 
                         variant="ghost" 
                         onClick={() => copyToClipboardText(JSON.stringify(getCleanJson(scene), null, 2), `scene-${idx}`)}
                     ...
                     <pre className="text-xs text-gray-400 font-mono overflow-x-auto p-5 bg-[#0a0a0a] custom-scrollbar max-h-[300px]">
                       {JSON.stringify(getCleanJson(scene), null, 2)}
                     </pre>
                   </div>
                 ))}
              </div>
