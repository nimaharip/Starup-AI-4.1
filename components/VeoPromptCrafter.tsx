...
import { copyToClipboard, delay, getGeminiApiKey, hasGeminiApiKey } from '../utils';
...
const VeoPromptCrafter: React.FC = () => {
    ...
    const handleGenerateVeoVideo = async () => {
        if (!hasGeminiApiKey()) {
            setVideoError("Silakan masukkan API Key Gemini Anda di menu Setelan API untuk mulai menggunakan fitur secara gratis.");
            return;
        }
        if (!promptEn) return;
        ...
        try {
            ...
            if (operation.error) {
                throw new Error(String(operation.error.message || "Unknown Veo API Error"));
            }
