...
import { base64ToArrayBuffer, pcmToWav, getGeminiApiKey, hasGeminiApiKey, mapVoiceNameToGeminiVoice } from '../utils';
...
const TTSGenerator: React.FC = () => {
    ...
    const handleGenerateVoiceOver = async () => {
        ...
        if (!hasGeminiApiKey()) {
            setNotificationMessage('Silakan masukkan API Key Gemini Anda di menu Setelan API untuk mulai menggunakan fitur secara gratis.');
            return;
        }
        ...
        try {
            const voiceToUse = selectedVoice ? mapVoiceNameToGeminiVoice(selectedVoice) : 'Kore';
            console.log(`[TTSGenerator] Mapping selected voice "${selectedVoice}" to Gemini prebuilt voice "${voiceToUse}"`);
